...
kable(colnames(tweetsDF), align = "c")
```
...
-   Table 1 displays the top 30 words used, with their counts.

```{r, eval = T}
knitr::kable(text_df %>% group_by(word) %>% 
  summarise(count = n()) %>% 
  arrange(desc(count)) %>% 
  head(30), align = "lc", caption = 'Top 30 words by count')
```

-   We can also plot the most commonly used words, to visualize frequency from another perspective.
...
-   Table 2 shows the top 10 tweets, by number of favorites.
-   Table 3 shows top 10 tweets, by number of retweets.
-   Also, we can look at top 10 user, based on favorite count on table 4.
-   Conversely, a table top 10 users, based on retweet count, is on table 5.
...
```
```{r, eval = T}
knitr::kable(head(favoriteDFUnique2, 10), align = "lc", caption = 'Most Popular Tweets by Favorite Count')
```
...
knitr::kable(head(retweetDFUnique2, 10), align = "lc", caption = 'Most Popular Tweets by RT Count')
```
...
library(knitr)
tweetsDF <- read.csv("C:/Users/jmgre/OneDrive/Documents/Grad School/DS501/Week 3/Case Study 1/tweetsDF.csv", header = TRUE, sep = ",")
userDF <- tweetsDF[,c("screenName","favoriteCount")]
userDFSort <- arrange(userDF, desc(favoriteCount))
userDFUnique <- unique(userDFSort, by = "screenName")
userDFUnique2 <-  userDFUnique %>%
  arrange(desc(favoriteCount))
```
```{r, eval = T}
knitr::kable(head(userDFUnique2, 10), align = "lc", caption = 'Top 10 Favorited Users')
```
```{r eval = T, include = F}
library(dplyr)
library(tinytex)
...
library(knitr)
tweetsDF <- read.csv("C:/Users/jmgre/OneDrive/Documents/Grad School/DS501/Week 3/Case Study 1/tweetsDF.csv", header = TRUE, sep = ",")
retweetDF <- tweetsDF[,c("screenName","retweetCount")]
retweetDFSort <- arrange(retweetDF, desc(retweetCount))
retweetDFUnique <- unique(retweetDFSort, by = "screenName")
retweetDFUnique2 <-  retweetDFUnique %>%
  arrange(desc(retweetCount))
```
```{r, eval = T}
knitr::kable(head(retweetDFUnique2, 10), align = "lc", caption = 'Top 10 Retweeted Users')
```
- Lastly, we have table 6, that lists the top ten hashtags, by number of occurrences.
```{r, eval = T}
library(dplyr)
library(stringr)
library(tidytext)
tweetsDF <- read.csv("C:/Users/jmgre/OneDrive/Documents/Grad School/DS501/Week 3/Case Study 1/tweetsDF.csv", header = TRUE, sep = ",")
tweetsDF$hashtags <- str_extract_all(tweetsDF$text, "#\\w+")
hashTags <- tweetsDF %>%
  select(hashtags) %>%
  tidyr::unnest(hashtags)
```
```{r}
knitr::kable(hashTags %>% group_by(hashtags) %>% 
               summarise(count = n()) %>% 
               arrange(desc(count)) %>% 
               head(10), align = "lc", caption = 'Top 10 hashtags by count')
```